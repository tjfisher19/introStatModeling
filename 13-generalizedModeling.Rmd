...
Although Poisson regression can be appropriate for *count* data, there are many cases when the highly restrictive property $\lambda = E[Y] = Var(Y)$ is violated. In situations such as this a different modeling approach is necessary. A generalization of Poisson regression is known as Negative Binomial Regression. It is based on properties of the Negative Binomial distribution that is typically covered in a formal Probability course. However, here we do not need all those details for our discussion. The key element for our understanding is that the Negative Binomial distribution allows for a second parameter, and thus loosens the restriction that $E[Y]=Var(Y)$. Formulaically the negative binomial model is the same equation as \@ref(eq:poissonModel) except an additional *dispersion* parameter is estimated. We walk through an example.
...
It takes a minute to comprehend everything in this plot. Visually it is difficult to distinguish any differences in gender. There does appear to be a relationship between attendance and the program (visually speaking "Vocational" and "Academic" appear to be lower on the days missed compared to "Gender"). We take a deeper dive at looking at this two categorical variables by comparing histograms.
...
Here we see three distinct shapes in the historgrams. For the "General" program it is more uniformly distributed (compared to the other two) with all recorded students having several days abseent. For "Academic" and "Vocational" we see more a decay type shape (also sometimes known as a Pareto shape). The "Academic" program definitely has more days absent compare to the "Vocational" program.
...
summary(attend.pois)
```
In the response we see that all the predictor variables are significant (all *p*-values are less than $10^{-6}$) but there is a vary glaring problem. The `Residual deviance` is recorded as 1746.8 on 309 degrees of freedom. This is a ratio value of `r 1746.8/309` which is much greater than 1. There are some major concerns about overdispersion. We could see this visually as well. Revisit the scatter plot of the data, with lower math scores, the range of the days absent looks larger than with higher math scores (an indication the variance changes given the mean response). 
...
* The model output now more closely matches our exploratory data analysis
* A quick check of the residual deviance suggest a better fit

### Infererence on predictor variables

In the previous fit we see that the `gender` variable does not appear significant. Let's fit a reduced model:
...
We see that the `math` variable does **not** significantly improve the model (*p*-value=0.0858).

### Plotting fitted model

Lastly, we will plot the fitted model. Even though the data used to fit the mode is technically 5 dimensional (days absent, math score, and 3 factor values), since we only have two covariate terms, we can use aesthetic opions in `ggplot` to plot the fitted model. We begin by creating some *fake* data that will results in a smooth fitted line:
...
You'll note this dataset has 300 observations and the math scores range from 1 to 99 in small incremenets, repeating for each of the three factor levels in `prog`. We then run this the `predict` function. Here we specify the `type="link"` so that the log of the terms is fit. This is important for building confidence bands in the below plot.
...
For each predicted response value (called `fit`) we have a standard error estimate (called `se.fit`). Using a standard $Z$-based confidence interval from your Intro Statistics course, we can build a confidence band around the fitted line.
...
Note the code specifying `lo` and `up`, it follows standard confidence intervals from your Intro Statistics course. The one thing that changes is application of the `exp()` function, here we are inverting the $\log$ link function used, see \@ref(eq:poissonModel).
