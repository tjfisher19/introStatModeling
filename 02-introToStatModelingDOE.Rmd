...
To answer to this question, the weight of the 20 mice has been measured before and after the treatment and is in the file `miceWeight.csv`. Below is some code to implement the test.

```{r ch2.4}
mice <- read.csv("miceWeight.csv")
head(mice)
t.test(weight ~ group, data=mice, paired=TRUE)
```

First we input the data with `read.csv` and then use the head function to get a sneak peak of the data (similar to `glimpse` earlier). Then we call the function `t.test` to perform the hypothesis test.
Note that similar to the two-sample *t*-test here we state the weight is a function of the group with the formula notation `weight ~ group` but here we state the data is paired with `paired=TRUE`. Lastly we note with a $p$-value of approximately zero we have strong evidence to support that the weight gain is non-zero.

**Graphical comparison**

To graphically compare paired data we must use some caution and consider the data is paired. Ultimately we are interested in the pairwise difference between observations. Thus, side-by-side boxplots are not fully appropriate since tehy do not account for the pairing. We can report a boxplot of the differences but this requires some data manipulation.
...
Another type of plot that can be handy for this sort of data (and will be more important later) is known as a profile plot. Here we use the `id` values to group data. Again we need to do some light data processing.
...
The above statement simply tells R to consider the numeric value `id` as a factor variable. We also specify the group ordering so that `before` is before `after`.

```{r ch2.7}
ggplot(mice) + 
  geom_line(aes(x=group, y=weight, group=id) ) +
  theme_minimal()
```

In both of these two plots it is clear there is significant weight gain during the study.
