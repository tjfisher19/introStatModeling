...
```{r, eval=FALSE}
uadata <- read.table("http://www.users.miamioh.edu/hughesmr/sta363/univadmissions.txt", header=TRUE)
```
```{r ch2.1}
uadata.trim <- uadata %>%
...
To demonstrates its implementation in R we consider another example. 10 mice received a dietary treatment during 3 months. We want to know whether the dietary treatment has an impact on the weight of the mice.

To answer to this question, the weight of the 10 mice has been measured before and after the treatment and is in the file `miceWeightwideform.csv`. Below is some code to implement the test.
