...
binary, we need to use a kind of model that handles binary outcomes correctly.
...
the errors is the binomial distribution. For count data, the Poisson distribution is a more appropriate choice. Most introductory statistics textbooks cover these distributions in detail.
...
While the response variable in a logistic regression is a 0/1 variable, the logistic regression equation, which is a linear equation, does not predict the 0/1 variable itself. Instead, logistic regression predicts the probability that an indicator variable is equal to 1. To be more precise, a logistic regression equation does not directly predict the probability that the indicator is equal to 1, i.e $P(Y = 1)$. Rather, it predicts the log odds that an observation will have an indicator equal to 1. 

**Odds.** The odds of an event is defined as the ratio of the probability that an event occurs to the probability that it fails to occur. Let $\pi$ denote the probability of an event of interest occurring, i.e., let $\pi = P(Y = 1)$. Then the odds of the event is given by $\pi/(1-\pi)$

The general form of a (multiple) logistic regression model always expressed in terms of the log odds:

$$\log\left(\frac{\pi}{1-\pi}\right) = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots \beta_k X_k$$
The link function $\log\left(\pi/(1-\pi)\right)$ is call the *logit* transformation. The logit ensures that our model will produce estimates of $\pi$ between 0 and 1. The quantity $\pi/(1 – \pi)$ is the odds of the event of interest occurring. The only downside of this transformation is that the linear predictor $\beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots \beta_k X_k$ is in terms of “log odds”, which is not easy to interpret. However, once we fit a model, we can back-transform in order to express our predictions as probabilities.

Here are some facts to keep in mind:

* Probabilities of events (e.g. $\pi$) must lie between 0 and 1, with 1⁄2 as a neutral value for which both outcomes are equally likely. The constraints at 0 and 1 make it impossible to construct a linear equation for predicting probabilities. 
...
Let $\pi$ = the probability that a postmenopausal woman is osteoporotic. Suppose our fitted model is

$$\log\left(\frac{\hat{\pi}{1-\hat{\pi}}\right) = -4353 + 0.038(age)$$
...
$$\frac{\hat{\pi}{1-\hat{\pi}} = e^{-4353 + 0.038(age)}$$
...
$$\hat{\pi} = \frac{e^{-4353 + 0.038(age)}}{1+e^{-4353 + 0.038(age)}}$$
...
We model $\pi = P(survival)$ as a function of UV level as follows:
...
The fitted logistic regression model is $\log\left(\frac{\hat{\pi}{1-\hat{\pi}}}\right) = −1.300 − 0.0233(UV~level)$.
...
$$\textrm{logit}(\pi) = \beta_0 + \beta_1(Class2) + \beta_3(Class3) + \beta_4(Crew) + \beta_5(Male) + \beta_6(Child) + \beta_7(Class2Male) + \beta_8(Class3Male) + \beta_9(CrewMale)$$
...
$$\frac{e^{3.558-3.885-4.233+2.863}}{1+e^{3.558-3.885-4.233+2.863}} = 0.154857$$
which is relativaly close to the $75/(75+387) = 0.162338$ that did survive.
