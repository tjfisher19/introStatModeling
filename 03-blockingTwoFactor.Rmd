...
A factorial structure in an experimental design is one in which there are two or more factors, and the levels of the factors are all observed in combination. For example, suppose you are testing for the effectiveness of three drugs (A, B, C) at three different dosages (5 mg, 10 mg, 25 mg). In a factorial design, you would observed a total of nine treatments, each of which consists of a combination of drug and dose (e.g. treatment 1 is drug A administered at 5 mg, treatment 2 is drug A administered at 10 mg, etc).
...
Note there is replication (i.e. multiple independent observations) in each treatment.
...
* $\beta_j$ is the "main effect" of the $i^\mathrm{th}$ level of factor $B$ on the mean response
...
First recall an important feature of designed experiments -- the analysis is determined by the experiment! In most cases of factorial design, the primary interest is to see if the two factors interact to determine the response! Therefore it is crucial to test the interaction term before assessing the effects of factor $A$ alone on the response, because if $A$ and $B$ interact, then we cannot separate their effects. In other words, if the interaction between $A$ and $B$ significantly influences the response, then the effect that factor $A$ has on the response changes depending on what the level of factor $B$.
...
First take a look at the alertness scores by treatment (note that we need to coerce dosage
to be an R factor):
...
There may be a gender effect: it appears that male responses are (on average) lower than females. However, the large spread in the 25 mg dosage
group for males may mask this effect.

We fit the interaction model in `aov()` using the usual means of specifying interactions. First, though, run a check on the residuals:

```{r 3.7}
twoway.fit <- aov(alertness ~ factor(dosage) + gender + factor(dosage):gender, data=twowaydrug)
autoplot(twoway.fit)
```
...
These consist of:
* The “grand mean” (overall mean, all data pooled together ... estimate of $\mu$)
...
These means are estimates from the data and are purely descriptive (like the boxplots were), but informative nonetheless. They can be obtained easily using the R function `model.tables()` and calling up the fitted interaction model:

```{r ch3.8}
model.tables(twoway.fit, "means")
```

We proceed to the tests:
...
**The lack of significant interaction is evidenced by the parallelism of these traces:** the dosage effect for females is depicted by the “gap” between the traces at the far left. Similarly, the dosage effect for the males is depicted by the gap between the traces at the far right. Note that these two “gaps” are of roughly the same size (which happens when the traces are parallel): this means that the effect of dose within females is about the same as the effect of dose within males.
...
Since the interaction term was non-significant and now we have visual support that the two factors do not interact, we may look at a reduced model that eliminates the interaction term and includes only main effects:
...
We note that the adjusted 95\% confidence overlap, this should not be surprising given we only had marginal (*p*-value$\approx$0.097) evidence that alertness was different between genders.
...
As stated before, there are ten replicate guinea pigs per treatment. The data appear in the R workspace `ToothGrowth` included in the `datasets` package. Here is the head of the dataframe:
...
Let’s bypass generating numeric descriptive statistics for the moment, and instead jump to an interaction plot of the length response. 
...
Once again, non-parallel traces may be the red flag. But, we need to run the ANOVA test for interaction to confirm if this is a significant effect. First, check assumptions:
...
This can all be done quite easily in the `emmeans` package. We include both comparisons below.
...
At a dose level of 0.5 mg, ascorbic acid delivery via orange juice produces significantly larger mean tooth growth than does delivery via Vitamin C. We determined this based on the the second set of output, where we see an adjusted *p*-value of 0.0021 comparing orange juice with Vitamin C. Similarly, at `dose=1.0`, there is still a different but not at `dose=2.0`.
