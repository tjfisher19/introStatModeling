...
```{r ch3-3, fig.align='center', fig.cap="Boxplot distribution of the baryley yeilds of `LocationYear` and barley `Variety`.",out.width='100%', fig.asp=0.55}
...
ggplot(fisherBarleyData) + 
  ...
  labs(x="Baryley Variety", y="Yield (bushels)", color="Location & Year") + 
  theme_minimal()
```

This plot in Figure \@(fig:ch3-3profile) is an improvement over the previous result but can be further tweaked for visual improvement.
...
ggplot(fisherBarleyData) + 
  ...
  labs(x="Baryley Variety", y="Yield (bushels)") + 
  theme_minimal()
```

From the profile plots in Figures \@(fig:ch3-3profile) and \@ref(fig:ch3-ImprovedProfile) it is clear that certain Location & Years were more fruitful than others, in terms of barley yield.
