...
In this chapter we will explore the two main anlayses for designed exerpiements -- a Block design and Two-factor experiment. The learning objective of this unit unclude:
...
The design is best described in terms of the agricultural field trials that gave birth to it. When conducting field trials to compare plant varieties (or whatever), there is concern that some areas of a field may be more fertile than others. So, if one were comparing three plant varieties, it would not be a good idea to use one variety in one location, another variety in a different location, and the third variety way out back because the effects of the varieties would be confounded with the natural fertility of the land.  
...
**An example.** You've already seen the concept of a block design! Consider the paired *t*-test included in Section \@ref(paired-t-test). There, the individual mice are essentially treated as blocks. We observe two responses within each block (or mouse).
...
to be an R factor since it is coded numerically):
...
We fit the interaction model in `aov()` by telling R to include an interaction term with the notation `factor1:factor2`. Below is the specific example for our study but first run a check on the residuals:
...
As alluded to before, there are ten replicate guinea pigs per treatment. The data appear in the R workspace `ToothGrowth` included in the `datasets` package. Here is the head of the dataframe:
