...
You can always force R to use the `dplyr` version by specifying the pacakge as in the below example:
...
```{r ch1-25_scatter, fig.align='center', fig.cap='Scatterplot showing the relationship between the variables `act` and `gpa.endyr1` in the `uadata` dataset.', out.width='75%'}
ggplot(uadata) + 
...
You should note in Figure \@ref(ch1-25_scatter) the overall positive relationship (higher ACT scores tend to correspond to higher GPAs). 
