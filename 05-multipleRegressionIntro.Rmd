...
We look to partition the variation in the data into (1) a systematic component, and (2) a non-systematic, or random, component.
...
Both variables are measured numeric values (compared to pre-determined treatments or categories). 
...
This model is considered *linear* because the relationship between the predictor variables, and the response is a linear combination.
When $k=1$, the equation reduces to that of a line.
When $k=2$, we are technically modeling a plane in 3-dimensional space and for $k\geq 3$, the equation defines a *hyperplane*.
...
The multiple regression model can be considered a generalization of a $k$ factor ANOVA model but instead of categorical inputs, we have numeric, or quantitative, inputs.

### Simple Linear Regression

A special case of the above model occurs when $k=1$ and the model reduces to
$$Y = \beta_0 + \beta_1 X + \varepsilon$$

This is known as the simple linear regression (SLR) model and is covered in most Intro Statistics courses.
It is fairly rare that a practicing Statistician will fit a SLR but we will utilize it to explain some key concepts.

It should be noted that multiple regression is in general a $k + 1$ dimensional problem, so it will usually not be feasible to graphically visualize a fitted model like we can with SLR (which only has 2-dimensions).
Even though visualization may not be possible, we can quantitatively explain properties in higher dimensions.
...
To explore this relationship in women, a nutritionist randomly selected 15 women from each 10-year age group beginning with age 40 and ending with age 79 and recorded their age (years) and muscle mass (kg).
...
Figure \@ref(fig:ch5-1) displays the data.
...
```{r ch5-14pairs, message=FALSE, fig.align='center', fig.cap='Pairs plot showing the relationship between right leg strength, left leg strengh, power and hang time.', fig.asp=1, out.width='80%'}
