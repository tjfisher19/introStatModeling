...
We now start the discussion of using the least squares simple linear regression model for the purpose of statistical inference about the parent population from which the sample was drawn.  Remember that by “inference,” we mean finding relevant confidence intervals or running hypothesis tests about various aspects of the modeled relationship.
...
This section is only meant to provide you with the means to run a cursory check of the assumptions in a simple linear regression problem.  We will deal with remedies for addressing assumption violations later in the course. 
...
More about Cook’s Distance later.


## Overall $F$-test for model signifance

Motivation for the whole-model test.  Before proceeding to investigating the effects of the individual predictors one at a time, we usually perform a “whole-model” test to first confirm that there is merit (i.e. “utility”) to the model.  This is in essence testing the following hypotheses:
...
One way of thinking about testing this null hypothesis is to think about fitting two models to the data, one using all the predictors (the ‘full” model), and the other using none of the predictors (a “reduced” null model), and seeing if there is a significant difference between them.  Thinking this way, the hypotheses may be rewritten again as:
    ...
    $H_0:$ the model is $Y = \beta_0 + \varepsilon$
    $H_a:$ the model is $Y = \beta_0 + \beta_1 X_1 + \beta_2 X_2 + \ldots + \beta_k X_k + \varepsilon$
    
Deriving a useful test statistic.  We now construct a test statistic that compares the residual variance of the two models.  We fit both the full $H_a$ model and the reduced $H_0$ model and obtain their respective residual sums of squares values.  It is a mathematical fact that $RSS_{H_a} \geq RSS_{H_0}$ (why?), so if the difference $RSS_{H_a}-RSS_{H_0}$ is small, then the fit of the reduced $H_0$ model is almost as good as the full $H_a$ model, so we would prefer the reduced model on the basis of simplicity. On the other hand, if the difference is large, then the superior fit of the full $H_a$ model would be preferred.  If we scale this difference by how much error variation was in the full model in the first place, then this suggests that something like

$$\frac{Reduction in error variance, H_0 model \rightarrow H_a model}{Error variance in H_a model} \leftrightarrow \frac{RSS_{H_a}-RSS_{H_0}}{RSS_{H_a}}$$

would be a potentially good test statistic.  

In practice, we will use what is known as **ANOVA F-statistic** to compare the numerator variance to the denominator variance:


$$F = \frac{\left(RSS_{H_a}-RSS_{H_0}\right)/\left(error df_{H_a} - error df_{H_0}\right)}{RSS_{H_a}/error df_{H_a}}$$
...
* If F is too large, we reject H0 in favor of Ha and conclude that at least one of the predictor variables is useful in determining the response (i.e. at least one $\beta_i \neq 0$).

* If F is small, fail to reject $H_0$ in favor of Ha and conclude that there is insignificant evidence to conclude that at least one of the predictor variables is useful in determining the response.

This type of test is called an analysis of variance (ANOVA) F-test.  This test is widely used in many types of statistical modeling, and especially in the analysis of designed experiments (we saw this before!).  The “whole-model” F-test for model utility is generally assessed first when fitting a model.  


```{example, label="appraisal_f", name="Property Appraisals"}
...
The $F$-statistic on 3 and 16 degrees of freedom is 46.72, with *p*-value < 0.0001. We reject $H_0$.  At least one predictor is useful for predicting sales price.  

Alternatively we could test this with the `anova()` function
...
This null hypothesis basically states that X and Y have no linear relationship.  Rejection of this null hypothesis offers statistical confirmation that there is a significant linear relationship between X and Y.  The test statistic for this is a t-statistic, and is given by

$$t=\frac{\textrm{point estimate}-\textrm{hypothesized value}}{\textrm{standard error of point estimate}} = \frac{b_i - 0}{SE_{b_1}}$$

and is provided in the `summary()` output in R.

```{example, label="appraisal_t_test", name="Property Appraisals"}
...
**Summary:** The appraised value of improvements is the most significant predictor of sales price.  Area of living space is marginally significant. 
...
So, here’s what happens by deleting landvalue as a predictor (we actually did this in the last lecture to see the effect on the β parameters):
...
However, as good statisticians we know that the above result isn’t sufficient.  Remember: it’s only an estimate of $\mu_{Y|X=65}$, the true mean muscle mass of women age 65.  We would like to construct confidence limits around this estimate to get an idea of its precision.  
...
The `predict()` extractor function in R will generate CIs and PIs from a linear model object according to your specifications for values of xp:
...
**Notes about the code.** We are using the layering feature in ``ggplot()`. First we plot the prediction interval as a `geom_ribbon` type with a lighter colored gray. Then the confidence band (since we know it will be narrower). Then the fitted line and original data points.
...
That is why when you conduct your own investigations, you should collect data from the entire region of predictor values of research interest to you.
...
```{r ch6.12, echo=FALSE, fig.width=3.25, fig.height=3.25}
set.seed(1)
x <- runif(500)
...
We close this chapter with a fully-worked out example, or case study, of a multiple regression model.
