...
You'll note these assumptions are nearly identical to those we discussed in experimental design.
...
**Example** Muscle mass.
Consider the age and muscle mass dataset from earlier.
...
Lastly, the QQ-Plot assesses normality of the $\varepsilon$ terms -- we wish for the empirical quantiles to closely match the theoretical, resulting in a fairly straight line. 
Here we note some slight deviation from the 45-degree line but nothing overly concerning.

**Warning:** When first studying and practicing statistics, many practitioners will fixate on a few single obsevations when exploring these plots. 
Don't let your eyes fool you and don't fixate on a few points that appear just a *little* different.
When they are greatly different, this can be a concern.
...
It is a mathematical fact that $RSS_{H_a} \geq RSS_{H_0}$ (thought exercise: why?).
If the difference $RSS_{H_a}-RSS_{H_0}$ is small, then the fit of the reduced "null" $H_0$ model is almost as good as the "full" $H_a$ model, so we would prefer the reduced model on the basis of simplicity.
