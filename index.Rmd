...
```{r dino, echo=FALSE, fig.height=4.5, fig.width=6, fig.align='center', message=FALSE, error=FALSE, warning=FALSE, fig.cap="Scatterplot resulting in the shape of a Tyrannusaurus with shaded Voronoi cells using the `ggvoronoi` package"}
library(tidyverse)
library(datasauRus)
library(ggvoronoi)
...
The book mixes statistical background with applications using the R Project for Statistical Computing. We have attempted to perform all data processing and analysis in this text using the *tidyverse*; thus all plots and functionality should follow the new grammar in R (*ggplot* and `%>%` type code).
